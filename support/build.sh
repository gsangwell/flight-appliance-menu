...
yum --installroot=$ROOTFS --nogpgcheck -y install openssh-server grub2 acpid deltarpm cloud-init cloud-utils-growpart gdisk at wget vim
# Remove unnecessary packages
UNNECESSARY="NetworkManager linux-firmware ivtv-firmware iwl*firmware"
...
add_drivers+=" nvme libata ata_generic pata_acpi ata_piix xen_blkfront "
EOF

# Recreate initramfs
KVER=$(echo /rootfs/boot/vmlinuz-3* | cut -f2- -d'-')
