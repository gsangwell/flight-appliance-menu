...
if ! [ -f /${CLUSTERPACK} ]; then
  printf "Full path to clusterpack plz" >&2
  exit 1
fi
...
if [ -z "$CLUSTERNAME" ]; then
  printf "Duff clusterpack" >&2
  exit 1
fi

if ( id "${CLUSTERNAME}" > /dev/null 2>&1 ); then 
  printf "Client by that name already exisits" >&2
  exit 1
fi
...
echo /etc/openvpn/clientscripts/$CLUSTERNAME.sh